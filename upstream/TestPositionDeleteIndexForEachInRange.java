...
/**
 * Verifies that {@link PositionDeleteIndex#forEachInRange(long, int, java.util.function.LongConsumer)}
 * returns exactly the positions that {@link PositionDeleteIndex#isDeleted(long)} reports as deleted
 * in the same range, in ascending order, for every implementation.
 */
public class TestPositionDeleteIndexForEachInRange {
  ...
  public void testMatchesIsDeletedOnRandomData() {
    ...
    for (int i = 0; i < 500; i++) {
      ...
      assertThat(collect(defaultIndex, start, length)).as("default at %s", start).isEqualTo(expected);
    }
  }
