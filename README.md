...
> | 50.00% | bitmap 100% | 496 (454–529) | 다시 오름 (분기 예측?) |
>
...
|---|---|---|---|---|
| **array** | 카디널리티 ≤ 4096 | 정렬된 `char[]` | 이진 탐색 O(log n) | 분기 예측 실패 다발, 포인터 추격 |
...
- **PMU 하드웨어 카운터는 AWS `.metal` 인스턴스에서만 신뢰 가능합니다.** 가상화 인스턴스에서는 `cycles`, `branch-misses`, `cache-misses` 대부분이 not-supported로 뜹니다.
